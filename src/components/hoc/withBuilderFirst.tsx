...
export interface WithBuilderFirstPublicProps {
  builderFirstMouseDown: any
  builderFirstMouseMove: any
}

interface WithBuilderFirstPrivateProps {
...
export type WithBuilderFirstProps = WithBuilderFirstPublicProps & WithBuilderFirstPrivateProps & WithHistoryProps


/**
 * レールを設置する機能を提供するHOC。このアプリの中核機能。
 */
export default function withBuilderFirst(WrappedComponent: React.ComponentClass<WithBuilderFirstProps>) {
  ...
  class WithBuilderFirstComponent extends React.Component<WithBuilderFirstProps, {}> {

    firstRailPosition: Point

    constructor (props: WithBuilderFirstProps) {
      super(props)
    ...
    mouseLeftDown(e: ToolEvent|any) {
      ...
      // 最初の一本目を設置しようとした
      if (this.props.phase === BuilderPhase.FIRST_POSITION) {
      ...
      } else if (this.props.phase === BuilderPhase.FIRST_ANGLE) {
    ...
    mouseMove = (e: ToolEvent|any) => {
      ...
      if (this.props.phase === BuilderPhase.FIRST_ANGLE) {
        const itemProps = RailFactory[this.props.selectedItem.name]()
    ...
    render() {
      return (
        <WrappedComponent
          {...this.props}
          builderFirstMouseDown={this.mouseDown}
          builderFirstMouseMove={this.mouseMove}
        />
      )
    }
  ...
  return connect(mapStateToProps, mapDispatchToProps)(WithBuilderFirstComponent)
}
